...
import {
  ...
  BarChart3, Users, Settings, Inbox, CalendarDays, MessageSquareQuote, Award,
...
const menuItems = [
  ...
  { icon: Award,           label: 'Trusted Clients', href: '/admin/trusted-clients' },
...
export function AdminSidebar({ isCollapsed, setIsCollapsed, isMobileOpen, onMobileClose }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <motion.aside
        initial={false}
        animate={{ width: isCollapsed ? '80px' : '280px', x: isMobileOpen ? 0 : (typeof window !== 'undefined' && window.innerWidth < 1024 ? -280 : 0) }}
      ...
      >
        ...
        <div className="h-24 flex items-center justify-between px-6 border-b border-white/5 bg-[#010205]">
          <AnimatePresence mode="wait">
            {!isCollapsed ? (
              <motion.div key="logo-full" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex items-center gap-3">
              ...
              <motion.div key="logo-small" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="mx-auto">
        ...
        {/* Navigation */}
        <div className="flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden py-6 px-4 space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <div className={cn(
                  ...
                  isActive ? "bg-purple-600/10 text-white" : "text-slate-400 hover:text-white hover:bg-white/5"
                )}>
                  {isActive && <div className="absolute left-0 w-1 h-6 bg-purple-500 rounded-full shadow-[0_0_10px_#9333EA]" />}
                  <div className={cn("p-2 rounded-lg transition-all duration-300", isActive ? "text-purple-400" : "group-hover:text-purple-400 group-hover:scale-110")}>
                    <item.icon size={20} className={cn("transition-all duration-300", isActive ? "drop-shadow-[0_0_8px_rgba(147,51,234,0.5)]" : "group-hover:drop-shadow-[0_0_8px_rgba(147,51,234,0.3)]")} />
                  </div>
